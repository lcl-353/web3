...
interface IERC20Receiver {
    function tokensReceived(address from, uint96 amount, uint256 data) external;
}
...
contract BaseERC20 {
    ...
    function transferWithCallback(address _to, uint96 _value, uint256 data) public returns (bool) {
        require(_value <= balances[msg.sender], "ERC20: transfer amount exceeds balance");
        ...
        if (isContract(_to)) {
            try NFTMarketV2(_to).tokensReceived(msg.sender, _value, data) {
                // success callback
...
contract NFTMarketV2 is IERC20Receiver {
    ...
    event Bought(uint256 indexed tokenId, address indexed buyer, uint96 price);
    ...
    function tokensReceived(
        ...
        require(
            ...
            //paymentToken.safeTransfer(item.sender, item.price),
            "Payment failed"
        );
